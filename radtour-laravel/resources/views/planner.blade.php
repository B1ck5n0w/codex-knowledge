...
<head>
  ...
  <style>
    ...
    * { box-sizing:border-box; }
    ...
    input[type=checkbox] { width:16px; height:16px; margin-right:7px; accent-color:var(--accent); vertical-align:-3px; } .weather-map-toggle { display:flex; align-items:center; gap:7px; margin:9px 0; padding:8px 10px; border:1px solid color-mix(in srgb,var(--accent) 42%,var(--line)); border-radius:9px; background:color-mix(in srgb,var(--accent) 8%,transparent); font-weight:680; cursor:pointer; } .weather-map-toggle input { margin:0; } button { width:100%; margin-top:8px; padding:10px 11px; border:0; border-radius:9px; color:#fff; background:var(--accent); cursor:pointer; box-shadow:0 2px 5px rgba(17,70,43,.16); transition:transform .16s,filter .16s,box-shadow .16s; } button:hover:not(:disabled) { filter:brightness(1.06); box-shadow:0 5px 13px rgba(17,70,43,.2); transform:translateY(-1px); } button:active:not(:disabled) { transform:translateY(0); } button.secondary { color:var(--text); background:transparent; border:1px solid var(--line); box-shadow:none; }
    ...
    /* Wetter bleibt eine Hinweis-Ebene: Die eigentliche Hin-/Rückroute soll stets sichtbar bleiben. */
    .leaflet-overlay-pane svg path.weather-rain-segment { stroke:#dc2626; stroke-width:4px; stroke-linecap:round; stroke-linejoin:round; stroke-dasharray:3 13; animation:weather-rain-flow 1.05s linear infinite; filter:drop-shadow(0 1px 1px rgba(75,0,0,.34)); pointer-events:stroke; }
...
<body>
  ...
  <main>
    <aside>
      ...
      <fieldset class="planner-step" data-step="4">
        ...
        <label class="weather-map-toggle" title="Blendet nur die roten Regenhinweise auf der Karte ein oder aus. Die Wetterbewertung bleibt sichtbar."><input id="weatherOverlayToggle" type="checkbox" checked><span>Regenrisiko auf Karte anzeigen</span></label>
        ...
        <small>Rote, punktierte Abschnitte markieren nur das Regenrisiko – die blaue und orange Route bleiben darunter sichtbar.</small>
  ...
  <script>
    ...
    let weatherOverlayVisible=true;
    function setWeatherOverlayVisible(visible) {
      weatherOverlayVisible=visible;
      [weatherRiskLayer,weatherRouteLayer].forEach(layer=>{
    ...
    async function overpass(query) {
      ...
      const aborter=new AbortController(), timer=window.setTimeout(()=>aborter.abort(),9000);
      try {
        ...
        if (error.name==='AbortError') throw new Error('Die Stopp-Suche antwortet gerade nicht schnell genug. Bitte erneut versuchen.');
    ...
    async function googleStopFallback(probes,corridor) {
      ...
      // wir maximal vier kleine, überlappende Suchkreise gleichmäßig auf der echten
      // Hinfahrt und filtern jedes Ergebnis danach nochmals auf den 100-m-Proben ab.
      const terms=[...new Set(selected)].slice(0,3), requestBudget=Math.min(4,Math.max(terms.length,terms.length*2)), requests=[];
      ...
      const settled=await Promise.allSettled(requests.map(async ({text,center})=>{ const response=await fetch('/api/places/search',{method:'POST',headers:{'Content-Type':'application/json','Accept':'application/json'},body:JSON.stringify({text,latitude:center.lat,longitude:center.lng,radius:3000})}); const payload=await response.json(); refreshGoogleUsage(); if (!response.ok) throw new Error(payload.message || 'Google Places ist nicht erreichbar.'); return (payload.places || []).map(place=>({name:place.displayName?.text || place.formattedAddress || text,lat:place.location?.latitude,lng:place.location?.longitude,tags:{...googleTags(place.primaryType),address:place.formattedAddress || '',googlePlaceId:place.id,googleSource:true}})); })); const batches=settled.filter(result=>result.status==='fulfilled').map(result=>result.value); if (!batches.length) throw new Error('Google Places ist nicht erreichbar.');
      const seen=new Set();
    ...
    function clearGoalSearch() { goalLayer.clearLayers(); goalMarkers=[]; document.getElementById('goalList').innerHTML=''; clearDestinationTextResults(); if (pendingMarker) { map.removeLayer(pendingMarker); pendingMarker=null; } goalOverlay.style.display='none'; radiusOverlay.style.display='none'; overlayPoint=null; radiusCenter=null; radiusMeters=0; goalSearchCenter=null; }
    ...
    function routeProgressMeters(point,probes) { let nearest=Infinity, nearestIndex=0; probes.forEach((probe,index)=>{ const distance=map.distance(point,probe); if (distance<nearest) { nearest=distance; nearestIndex=index; } }); return nearestIndex*100; }
    function curatedStopRows(rows,probes) {
      const totalMeters=Math.max(100,(probes.length-1)*100), maximum=totalMeters<18000?2:totalMeters<36000?3:4;
      const preferredBySection=[['Spielplatz','Sehenswürdigkeit','Tiere / Tiergehege','See / Badestelle'],['Café / Eiscafé','Biergarten','Restaurant','Picknick / Grillplatz','Öffentliche Toilette'],['See / Badestelle','Sehenswürdigkeit','Spielplatz','Öffentliche Toilette']];
      const candidates=rows.map(place=>({...place,routeProgress:routeProgressMeters(place,probes)})).sort((a,b)=>a.routeProgress-b.routeProgress||a.routeDistance-b.routeDistance);
      const recommended=[], minimumSpacing=Math.max(2800,totalMeters/(maximum+1)*.62);
      while (recommended.length<maximum) {
        let best=null, bestScore=-Infinity;
        candidates.forEach(place=>{
          if (recommended.includes(place)) return;
          const kind=placeKind(place.tags), section=Math.min(2,Math.floor(place.routeProgress/totalMeters*3));
          if (recommended.some(selected=>placeKind(selected.tags)===kind)) return;
          const nearestStop=recommended.length?Math.min(...recommended.map(selected=>Math.abs(selected.routeProgress-place.routeProgress))):Infinity;
          if (nearestStop<minimumSpacing) return;
          const sectionIsNew=!recommended.some(selected=>Math.min(2,Math.floor(selected.routeProgress/totalMeters*3))===section);
          const score=(preferredBySection[section].includes(kind)?45:12)+(sectionIsNew?24:0)-place.routeDistance*.07+(place.tags?.website?4:0)+(place.tags?.opening_hours?3:0);
          if (score>bestScore) { best=place; bestScore=score; }
        });
        if (!best) break;
        best.recommended=true; recommended.push(best);
      }
      const remaining=candidates.filter(place=>!recommended.includes(place));
      return [...recommended.sort((a,b)=>a.routeProgress-b.routeProgress),...remaining].slice(0,12);
    }
    function stopRouteLabel(place) { const km=Math.max(.1,(place.routeProgress || 0)/1000).toFixed(1); return 'bei km '+km+' · '+Math.round(place.routeDistance)+' m neben der Hinfahrt'; }
    ...
    function groupStopCards(rows,list) { if (!rows.length) return; const cards=[...list.querySelectorAll('.place-card')], entries=rows.map((place,index)=>({place,index,card:cards[index]})), icons={'Café / Eiscafé':'coffee','Biergarten':'beer','Restaurant':'utensils','Sehenswürdigkeit':'binoculars','Spielplatz':'toy-brick','Picknick / Grillplatz':'sandwich','Öffentliche Toilette':'toilet','See / Badestelle':'waves','Tiere / Tiergehege':'rabbit'}; list.innerHTML=''; const appendGroup=(label,items,open,icon='map-pin',recommended=false)=>{ if (!items.length) return; const group=document.createElement('details'); group.className='stop-group'+(recommended?' recommended-stop-group':''); group.open=open; const summary=document.createElement('summary'); summary.innerHTML='<i data-lucide="'+icon+'" aria-hidden="true"></i><span>'+safe(label)+'</span><b class="stop-group-count">'+items.length+'</b>'; const content=document.createElement('div'); content.className='stop-group-list'; items.forEach(entry=>content.append(entry.card)); group.append(summary,content); list.append(group); }; const recommended=entries.filter(entry=>entry.place.recommended); appendGroup('Für eure Tour empfohlen',recommended,true,'sparkles',true); const groups=new Map(); entries.filter(entry=>!entry.place.recommended).forEach(entry=>{ const kind=placeKind(entry.place.tags); if (!groups.has(kind)) groups.set(kind,[]); groups.get(kind).push(entry); }); groups.forEach((items,kind)=>appendGroup(kind,items,!recommended.length,icons[kind] || 'map-pin')); setTimeout(()=>window.lucide?.createIcons({attrs:{'aria-hidden':'true'}}),0); }
    async function findStops() {
      if (!start || !destinationData()) { status.textContent='Bitte zuerst Start und Ziel auswählen.'; return; }
      if (!forwardCoords.length && !await prepareForwardRoute()) return;
      const corridor=stopCorridorMeters(), discoverySpacing=Math.min(1000,Math.max(500,corridor*1.5)); setStopSearchLoading(true); status.textContent='Passende Stopps werden entlang der Hinfahrt im '+corridor+'-m-Korridor gesucht …'; clearStopMarkers(); stopCandidateState=[]; selectedStops=[]; document.getElementById('stopList').innerHTML='<div class="stop-search-progress">Suche entlang der echten Hinfahrt – maximal '+corridor+' m neben der Route.</div>'; const sample=routeSamplesEvery(forwardCoords,discoverySpacing), strictProbes=routeSamplesEvery(forwardCoords,100); const categories=[]; if (document.getElementById('includeCafe').checked) categories.push('nwr["amenity"~"cafe|ice_cream"]'); if (document.getElementById('includeBeer').checked) categories.push('nwr["amenity"="biergarten"]'); if (document.getElementById('includeRestaurant').checked) categories.push('nwr["amenity"="restaurant"]'); if (document.getElementById('includeSights').checked) categories.push('nwr["tourism"="viewpoint"]'); if (document.getElementById('includePlaygrounds').checked) categories.push('nwr["leisure"="playground"]'); if (document.getElementById('includePicnic').checked) categories.push('nwr["tourism"="picnic_site"]','nwr["leisure"="picnic_table"]','nwr["amenity"="bbq"]'); if (document.getElementById('includeToilets').checked) categories.push('nwr["amenity"="toilets"]'); if (document.getElementById('includeWater').checked) categories.push('nwr["leisure"="swimming_area"]','nwr["natural"="beach"]','nwr["natural"="water"]["access"!="private"]'); if (document.getElementById('includeAnimals').checked) categories.push('nwr["tourism"="zoo"]','nwr["animal"]');
      const renderStops=(candidateRows,notice)=>{ const rows=curatedStopRows(candidateRows,strictProbes), list=document.getElementById('stopList'); rows.forEach((p,i)=>previewPlaces.set('stop-image-'+i,p)); list.innerHTML=rows.map((p,i)=>'<div class="place-card">'+carouselHtml('stop-image-'+i,p.name,p.tags)+'<label><input type="checkbox" value="'+i+'"><strong>'+safe(p.name)+'</strong><span class="place-kind">'+placeKind(p.tags)+'</span><small>'+stopRouteLabel(p)+'</small>'+photoNote(p.tags)+websiteLink(p.tags)+'</label></div>').join('') || '<small>Keine passenden Stopps innerhalb von '+corridor+' m neben der Hinfahrt gefunden.</small>'; const inputs=[...list.querySelectorAll('input')]; const version=++stopSearchVersion; rows.forEach((p,i)=>{ const pin=stopMarker(p,i,inputs[i],version); stopMarkers.push(pin); stopCandidateState.push({place:p,pin,input:inputs[i]}); }); inputs.forEach(input=>input.addEventListener('change',()=>{ selectedStops=inputs.filter(x=>x.checked).map(x=>rows[Number(x.value)]); status.textContent=selectedStops.length+' Stopp(s) gewählt – Hinfahrt wird neu berechnet …'; calculate(); })); hydrateCarousels(rows,'stop-image'); status.textContent=notice(rows.length); };
      try { if (!categories.length) { status.textContent='Bitte mindestens eine Stopp-Kategorie auswählen.'; return; } const coordinates=sample.map(p=>p.lat+','+p.lng).join(','), around='around:'+corridor+','+coordinates, clauses=categories.map(category=>category+'('+around+');').join(''); const raw=await overpass('[out:json][timeout:12];('+clauses+');out center 300;'); const seen=new Set(), candidates=raw.map(e=>({...pointOf(e),name:nameOf(e),tags:e.tags||{}})).filter(p=>p.lat&&p.lng&&!seen.has(p.lat.toFixed(4)+p.lng.toFixed(4))&&seen.add(p.lat.toFixed(4)+p.lng.toFixed(4))).map(p=>({...p,routeDistance:strictCorridorDistance(p,strictProbes)})).filter(p=>isWithinStopCorridor(p,strictProbes,corridor)); renderStops(candidates,count=>count+' passende Stopps innerhalb von '+corridor+' m. Die besten, vielfältigen Empfehlungen stehen zuerst; weitere Orte sind aufklappbar.'); } catch(e) { const list=document.getElementById('stopList'); list.innerHTML='<div class="stop-search-progress"><span>OpenStreetMap reagiert gerade nicht schnell genug.</span><button id="loadGoogleStops" class="secondary" type="button">Google-Stopps ergänzen</button></div>'; const googleButton=document.getElementById('loadGoogleStops'); googleButton.addEventListener('click',async()=>{ googleButton.disabled=true; googleButton.textContent='Google-Suche läuft …'; try { const candidates=await googleStopFallback(strictProbes,corridor); renderStops(candidates,count=>count?'Google Places: '+count+' passende Stopps sind kuratiert dargestellt.':'Keine passenden Google-Places-Stopps innerhalb von '+corridor+' m gefunden.'); } catch (fallbackError) { status.textContent='Google-Ausweichsuche: '+fallbackError.message; googleButton.disabled=false; googleButton.textContent='Google-Stopps erneut versuchen'; } }); status.textContent='OpenStreetMap reagiert gerade nicht schnell genug. Google wird nicht automatisch abgefragt.'; }
    }
    ...
    async function prepareForwardRoute() {
      if (forwardCoords.length) return true;
      const end=destinationData(); if (!start || !end) return false;
      const preparationId=++routeCalculationVersion; status.textContent='Hinweg für die Stopp-Suche wird vorbereitet …';
      try {
        const data=await routeRequest([[start.lng,start.lat],[end.lng,end.lat]],'recommended'), feature=data.features?.[0];
        if (preparationId!==routeCalculationVersion || !feature) return false;
        forwardCoords=feature.geometry.coordinates.map(point=>({lat:point[1],lng:point[0]}));
        if (outbound) map.removeLayer(outbound); outbound=emphasizedRoute({type:'FeatureCollection',features:[feature]},'outbound');
        map.fitBounds(L.latLngBounds(forwardCoords).pad(.15)); status.textContent='Hinweg ist vorbereitet. Passende Stopps werden nun im gewählten Korridor gesucht …';
        return true;
      } catch (error) { status.textContent='Hinweg konnte nicht vorbereitet werden: '+error.message; return false; }
    }
    ...
    document.getElementById('weatherOverlayToggle').addEventListener('change',event=>setWeatherOverlayVisible(event.target.checked));
